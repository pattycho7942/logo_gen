...
| `FRONTEND_ORIGINS` | CORS 허용 origin, 콤마로 여러 개 지정 가능 (기본 `http://localhost:5173,https://logo-gen-ten.vercel.app,https://logo-1wkqusnzm-patty1103.vercel.app,https://logo-gen-git-main-patty1103.vercel.app`) — Vercel의 `<프로젝트>-git-<브랜치>-<팀>.vercel.app` 형태 주소는 브랜치별로 고정이라 안정적이고, 해시가 들어간 배포별 주소(`logo-1wkqusnzm-...`)는 재배포마다 바뀔 수 있어 바뀌면 이 값도 업데이트해야 합니다 | - |
